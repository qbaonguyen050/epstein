...
class EnemyAI {
    constructor(game) {
        ...
        // 特朗普的图片配置（使用现有图片作为回退）
        this.trumpImages = {
            'cam10': 'assets/images/scarytrump.png',
            'cam11': 'assets/images/scarytrump.png',
            'cam9': 'assets/images/scarytrump.png',
            'cam8': 'assets/images/scarytrump.png',
            'cam7': 'assets/images/scarytrump.png',
            'cam6': 'assets/images/scarytrump.png',
            'cam5': 'assets/images/scarytrump.png',
            'cam1': 'assets/images/scarytrump.png',
            'cam2': 'assets/images/scarytrump.png',
            'cam3': 'assets/images/scarytrump.png',
            'cam4': 'assets/images/scarytrump.png',
        };
    ...
    // 获取有效的AI等级（包含小时加成）
    getEffectiveAILevel(baseLevel) {
        let level = baseLevel;
        const hour = this.game.state.currentTime;

        // 每小时AI等级+1 (2AM以后)
        if (hour >= 2) level += 1;
        if (hour >= 4) level += 1;
        if (hour >= 5) level += 2; // 5AM 变得非常危险

        return Math.min(20, level);
    }

    // 检查是否受到玩家观察的干扰（Stalling）
    isBeingWatched(enemyType) {
        if (!this.game.state.cameraOpen || this.game.state.cameraFailed) return false;

        const currentCam = this.game.state.currentCamera;
        if (enemyType === 'epstein') {
            return this.epstein.currentLocation === currentCam;
        } else if (enemyType === 'trump') {
            return this.trump.currentLocation === currentCam;
        }
        return false;
    }
    ...
    checkMovement() {
        ...
        // 计算当前有效AI等级
        let effectiveLevel = this.getEffectiveAILevel(this.epstein.aiLevel);

        // 如果正在被观察，移动概率减半 (Stalling mechanic)
        if (this.isBeingWatched('epstein')) {
            effectiveLevel = Math.floor(effectiveLevel / 2);
            console.log(`Epstein is being watched at ${this.epstein.currentLocation}, effective AI level reduced to ${effectiveLevel}`);
        }
        ...
        // 如果随机数 <= AI等级，移动成功
        if (randomNumber <= effectiveLevel) {
            this.moveToNextLocation();
        } else {
            console.log(`Epstein failed movement check: ${randomNumber} > ${effectiveLevel}`);
        }
    }
    ...
    checkTrumpMovement() {
        ...
        // 计算当前有效AI等级
        let effectiveLevel = this.getEffectiveAILevel(this.trump.aiLevel);

        // 如果正在被观察，移动概率减半
        if (this.isBeingWatched('trump')) {
            effectiveLevel = Math.floor(effectiveLevel / 2);
            console.log(`Trump is being watched at ${this.trump.currentLocation}, effective AI level reduced to ${effectiveLevel}`);
        }
        ...
        if (randomNumber <= effectiveLevel) {
            ...
            console.log(`Trump failed movement check: ${randomNumber} > ${effectiveLevel}`);
        }
    }
    ...
    updateHawkingWarningDisplay() {
        ...
        // 设置警告图片（如果原图缺失，回退到星星图标）
        if (this.hawking.warningLevel === 1) {
            warningIcon.src = 'assets/images/star.png';
    ...
    triggerHawkingMissileJumpscare() {
        ...
        // 创建导弹图片（从霍金位置飞向玩家，缺失则用风扇图标代替）
        const missileImg = document.createElement('img');
        missileImg.src = 'assets/images/fa3.png';
        ...
        // 爆炸原图缺失，暂时使用 winscreen 背景
        explosionImg.style.backgroundImage = 'url(assets/images/winscreen.png)';
        explosionImg.style.backgroundSize = 'cover';
